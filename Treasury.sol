...
contract Treasury is ContractGuard ,VRFConsumerBaseV2{
    ...
    /* ========== CHAINLINK VARIABLES ========== */
     ...
     mapping(uint256 => uint256) private epochPeriods;
    ...
    function nextEpochPoint() public view returns (uint256) {
        ...
        if(epochPeriods[epoch] != 0 && epochPeriods[epoch]!= ROLL_IN_PROGRESS)
        {
            multiplierHr = multiplierHr* epochPeriods[epoch];
            return startTime.add(epoch.mul(multiplierHr));
        }
    ...
    function initialize(
        ...
        address _boardroom,
        uint256 _startTime
    ) public VRFConsumerBaseV2(vrfCoordinator)  notInitialized {
        grape = _grape;
        gbond = _gbond;
        wine = _wine;
        ...
        //initialize epoch randomization
    ...
    function requestRandomNumber(uint256 _epoch) public onlyOwner returns (uint256 requestId) {
        ...
        emit RequestRandomnessFulfilled(requestId, numWords);
    }
    ...
    function fulfillRandomWords(
        ...
        uint256 randVal = randomness % (randmax - randmin + 1) + randmin;
